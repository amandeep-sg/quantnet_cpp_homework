...
Line::Line() : start_point(Point()), end_point(Point()) // initialise line with start point (0.00, 0.00) and end point (0.00, 0.00)
{
    cout << "Constructor called: New line created!";
};

Line::Line(Point &a, Point &b) : start_point(a), end_point(b) // overload the constructor to initialize line with point a & b
{
    cout << "Constructor called: New line created with start " << a.ToString() << " and end " << b.ToString() << endl;
};

Line::Line(const Line &other) : start_point(other.start_point), end_point(other.end_point) // copy constructor
{
...
{
    stringstream ss;
    ss << "Line Start " << start_point.ToString() << " | Line End " << end_point.ToString() << endl;
    return ss.str();
}